...
import KBG.Auth ( discovery, requireSession, loadEnvCredentials, EnvCredentials(..), parseCookies)
import KBG.Styles (formFields, mainPage, notFoundPage)
import KBG.DB (withDb, initDb, saveSubmission, getAllSubmissions)
...
main = do
    ...
    let settings = setPort envCreds.port
                 ...
                 $ setLogger logRequest
                   defaultSettings
    runSettings settings (requireSession manager disc (app envCreds))
...
app envCreds req res = case (requestMethod req, pathInfo req) of
    ("GET",  [])    -> res $ htmlResponse status200 (renderHtml mainPage)
    ("POST", [])    -> do
        (params, _) <- parseRequestBody lbsBackEnd req
        let fields = map (\(k,v) -> (C8.unpack k, C8.unpack v)) params
        let userId = getUserId req
        putStrLn $ "[db] Saving submission for user_id " ++ show userId
        withDb envCreds.databaseURL $ \conn -> saveSubmission conn userId fields
        putStrLn $ "[db] Saved submission for user_id " ++ show userId
...
getUserId :: Request -> Int
getUserId req = fromMaybe 0 $ do
